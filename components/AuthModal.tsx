...
export default function AuthModal({ mode: initialMode, onClose, onSuccess }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
      <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-md p-6 sm:p-8 shadow-2xl relative">
        <button
          onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {error && (
            <div className="p-3 rounded-lg bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs leading-relaxed">
              ⚠️ {error}
            </div>
          )}
          ...
          {mode === "signup" && (
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-950/80 border border-white/10 rounded-lg px-3.5 py-2.5 text-sm text-white focus:outline-none focus:border-brand-500 transition-colors"
              />
            </div>
          )}
              ...
              <div className="p-3 rounded-xl bg-slate-950/60 border border-emerald-500/20 space-y-2">
                <label className="flex items-start gap-2.5 cursor-pointer">
                  <input
                    ...
                    className="mt-0.5 w-4 h-4 rounded text-brand-600 focus:ring-brand-500 border-slate-700 bg-slate-900"
                  />
